...
import { RedeCreditOrderModel } from '@models/file-processing.models';
import { formatCurrency } from '../file-processing-ui';

@Component({
  selector: 'cs-rede-credit-orders',
  standalone: true,
  imports: [CommonModule, RouterLink, ButtonModule, CardModule, TableModule],
  templateUrl: './rede-credit-orders.component.html',
  styleUrl: './rede-credit-orders.component.scss',
})
export class RedeCreditOrdersComponent {
  private readonly service = inject(FileProcessingService);

  protected readonly rows = signal<RedeCreditOrderModel[]>([]);
  ...
  protected load(event: TableLazyLoadEvent = { first: 0, rows: this.pageSize() }): void {
    ...
    this.service
      .listRedeCreditOrders({ page, size: rows, sort: `${sortField},${sortOrder}` })
      .subscribe({
        next: (response) => {
