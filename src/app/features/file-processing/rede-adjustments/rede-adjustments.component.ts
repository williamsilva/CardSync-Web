...
import { RedeAdjustmentModel } from '@models/file-processing.models';
import { formatCurrency } from '../file-processing-ui';
import { CsTagComponent } from '@shared/ui';

@Component({
  selector: 'cs-rede-adjustments',
  standalone: true,
  imports: [CommonModule, RouterLink, ButtonModule, CardModule, TableModule, CsTagComponent],
  templateUrl: './rede-adjustments.component.html',
  styleUrl: './rede-adjustments.component.scss',
})
export class RedeAdjustmentsComponent {
  private readonly service = inject(FileProcessingService);

  protected readonly rows = signal<RedeAdjustmentModel[]>([]);
  ...
  protected load(event: TableLazyLoadEvent = { first: 0, rows: this.pageSize() }): void {
    ...
    this.service
      .listRedeAdjustments({ page, size: rows, sort: `${sortField},${sortOrder}` })
      .subscribe({
        next: (response) => {
