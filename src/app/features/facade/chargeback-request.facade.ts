...
import { ChargebackRequestApiService } from '@features/service/adjustment-chargeback-request.api.service';
import {
  ChargebackRequestApiModel,
  ChargebackRequestTotalsModel,
} from '@models/chargeback-request.model';

type LastQuery = ListQueryDto<ChargebackRequestAdvancedFilters>;

@Injectable({ providedIn: 'root' })
export class ChargebackRequestFacade {
  private readonly api = inject(ChargebackRequestApiService);
  ...
  private readonly _data = signal<ChargebackRequestApiModel[]>([]);
  private readonly _totals = signal<ChargebackRequestTotalsModel | null>(null);
