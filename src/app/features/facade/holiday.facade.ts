...
import { HolidayModel, HolidayCreateInput, HolidayUpdateInput } from '@models/holiday.models';
import { HolidayAdvancedFilters } from '@features/filter/holiday.filters';
import { HolidayApiService } from '@features/service/holiday.api.service';
import { ListQueryDto } from '@shared/features/list-query/list-query.types';

type LastQuery = ListQueryDto<HolidayAdvancedFilters>;

@Injectable({ providedIn: 'root' })
export class HolidayFacade {
  private readonly api = inject(HolidayApiService);

  private readonly _total = signal(0);
  private readonly _loading = signal(false);
  private readonly _loadedOnce = signal(false);
  private readonly _data = signal<HolidayModel[]>([]);
  private readonly _lastQuery = signal<LastQuery | null>(null);

  readonly holidays = this._data.asReadonly();
  ...
  getById(id: string): Observable<HolidayModel> {
    return this.api.getById(id);
  }

  create(input: HolidayCreateInput): Observable<HolidayModel> {
    this._loading.set(true);
  ...
  update(id: string, input: HolidayUpdateInput): Observable<HolidayModel> {
    this._loading.set(true);
