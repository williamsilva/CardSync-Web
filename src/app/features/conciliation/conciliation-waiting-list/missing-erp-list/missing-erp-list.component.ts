  ...
  implements OnInit, AfterViewInit
{
  ...
  protected readonly batchRows = computed(() => this.selectedCreateRows());
  ...
  protected readonly selectedCreateRows = computed(() =>
    this.selectedRows().filter((row) => this.canCreateErp(row)),
  );

  protected readonly headerEligibleRows = computed(() =>
    this.sales().filter((row) => this.canCreateErp(row)),
  );
  ...
  protected override loadPage(
    ...
    this.facade.clearTotals();
    this.facade.load('missing-erp', query);
  }
  ...
  protected canCreateErp(row: ConciliationWaitingModel | null | undefined): boolean {
  ...
  protected openBatchDialog(): void {
    const rows = this.selectedCreateRows();

    if (!rows.length) {
      this.toast.warn(
        this.i18n.tUi('conciliation.noEligibleSelectedTitle'),
        this.i18n.tUi('conciliation.missingErp.batch.noEligibleSelectedDetail'),
      );
      return;
    }

    this.pendingBatchAction.set('CREATE_ERP');
  ...
  protected confirmBatchAction(_payload?: any): void {
    ...
    this.facade
      .createErpFromAcquirerBatch(ids)
      ...
      .subscribe({
        ...
        error: () => {
          this.toast.error(
            ...
            this.i18n.tUi('conciliation.missingErp.batch.createErrorDetail'),
          );
        },
      });
  }

  protected confirmCreateErp(row: ConciliationWaitingModel): void {
    const acquirerId = this.transactionId(row);

    if (!acquirerId) {
      this.toast.warn(
        this.i18n.tUi('conciliation.missingErp.single.acquirerNotFoundTitle'),
        this.i18n.tUi('conciliation.missingErp.single.acquirerNotFoundDetail'),
      );
      return;
    }

    this.pendingConfirmAction.set('CREATE_ERP_SINGLE');
  ...
  protected confirmAction(_payload?: any): void {
    ...
    if (!row || !acquirerId || this.resolving()) {
      return;
    }
  ...
  protected batchDescription(): string {
    return this.i18n.tUi('conciliation.missingErp.batch.description');
  }

  protected batchActionSuffix(): string {
    return this.i18n.tUi('conciliation.missingErp.batch.actionSuffix');
  }

  protected batchConfirmLabel(): string {
    return this.i18n.tUi('conciliation.missingErp.batch.confirmLabel');
  }

  protected confirmActionDescription(): string {
    return this.i18n.tUi('conciliation.missingErp.single.description');
  }

  protected confirmActionSuffix(): string {
    return this.i18n.tUi('conciliation.missingErp.single.actionSuffix');
  }
  ...
  private createErpFromAcquirer(acquirerTransactionId: string): void {
    if (this.resolving()) return;
    ...
    this.facade
      ...
      .subscribe({
        ...
        error: () => {
          this.toast.error(
            this.i18n.tUi('conciliation.missingErp.single.createErrorTitle'),
            this.i18n.tUi('conciliation.missingErp.single.createErrorDetail'),
          );
        },
      });
  }
  ...
  private handleResolutionSuccess(result: ErpAcquirerResolutionResultModel): void {
    ...
    this.toast.success(
      this.i18n.tUi('conciliation.missingErp.single.createdTitle'),
      result?.message || this.i18n.tUi('conciliation.missingErp.single.createdDetail'),
      7000,
    );
  }
  ...
  private transactionId(row: ConciliationWaitingModel | null | undefined): string | null {
    const raw = row as
      | (ConciliationWaitingModel & {
          ...
          transactionAcqId?: string | null;
          transactionAcquirerId?: string | null;
          acqTransactionId?: string | null;
        })
      | null
      | undefined;

    return (
      raw?.acquirerTransactionId ??
      raw?.transactionAcqId ??
      raw?.transactionAcquirerId ??
      raw?.acqTransactionId ??
      raw?.id ??
      null
    );
  }
