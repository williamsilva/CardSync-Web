  ...
  implements OnInit, AfterViewInit
{
  ...
  protected override loadPage(
    ...
    this.facade.clearTotals();
    this.facade.load('other-divergences', query);
  }
  ...
  protected canCompare(row: ConciliationWaitingModel | null | undefined): boolean {
    return !!row?.erp?.id && !!row?.acquirerTransaction?.id;
  }

  protected compareTooltip(row: ConciliationWaitingModel | null | undefined): string {
    if (this.canCompare(row)) {
      return this.i18n.tUi('conciliation.otherDivergences.compareTooltip');
    }

    return this.i18n.tUi('conciliation.otherDivergences.compareUnavailableTooltip');
  }

  protected openComparison(row: ConciliationWaitingModel, event?: Event): void {
    event?.preventDefault();
    event?.stopPropagation();

    const erpId = row.erp?.id;
    const acquirerId = row?.acquirerTransaction?.id;

    if (!erpId || !acquirerId) {
      this.toast.warn(
        this.i18n.tUi('conciliation.otherDivergences.compareUnavailableTitle'),
        this.i18n.tUi('conciliation.otherDivergences.compareUnavailableDetail'),
      );
      return;
    }
    ...
    this.comparisonDialogVisible.set(true);
    this.comparing.set(true);

    this.facade
      .compareErpVsAcquirer(erpId, acquirerId)
      .pipe(finalize(() => this.comparing.set(false)))
      .subscribe({
        next: (result) => this.comparison.set(result),
        error: () => {
          this.toast.error(
            this.i18n.tUi('conciliation.otherDivergences.compareErrorTitle'),
            this.i18n.tUi('conciliation.otherDivergences.compareErrorDetail'),
          );
        },
      });
  }
  ...
  protected confirmManualReconciliation(truthSource: ErpAcquirerTruthSource): void {
    ...
    if (!erpId || !acquirerId || this.resolving()) {
      return;
    }
    ...
    this.facade
      .reconcileErpVsAcquirerManually(erpId, acquirerId, truthSource)
      ...
      .subscribe({
        next: (result) => {
          ...
          this.toast.success(
            this.i18n.tUi('conciliation.otherDivergences.manualResolvedTitle'),
            result?.message || this.i18n.tUi('conciliation.otherDivergences.manualResolvedDetail'),
            7000,
          );
        },
        error: () => {
          this.toast.error(
            this.i18n.tUi('conciliation.otherDivergences.manualErrorTitle'),
            this.i18n.tUi('conciliation.otherDivergences.manualErrorDetail'),
          );
        },
      });
  }
