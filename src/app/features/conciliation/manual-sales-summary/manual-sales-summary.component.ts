...
import {
  ...
  STATUS_CODE_MAP,
  modalityEnumLabel,
} from '@features/models/enums/modality.enum';
...
export class ManualSalesSummaryComponent implements OnInit {
  ...
  protected readonly saleSummaryFacade = inject(SaleSummaryFacade);
  protected readonly creditOrderFacade = inject(CreditOrderFacade);
  ...
  constructor() {
    effect(() => {
      const establishments = this.establishmentFacade.activeOptions();
      const acquirers = this.acquirerFacade.activeOptions();
      const companies = this.companyFacade.activeOptions();

      untracked(() => {
        if (establishments.length === 1 && !this.form.value.establishmentId) {
          this.form.patchValue({ establishmentId: establishments[0].id });
        }
        if (acquirers.length === 1 && !this.form.value.acquirerId) {
          this.form.patchValue({ acquirerId: acquirers[0].id });
          this.csvAcquirerId.set(acquirers[0].id);
        }
        if (companies.length === 1 && !this.form.value.companyId) {
  ...
  ngOnInit(): void {
    this.form.get('establishmentId')!.valueChanges.subscribe((id) => {
      if (!id) return;
      const establishment = this.establishmentFacade.activeOptions().find((e) => e.id === id);
  ...
  private mapRedeModality(modalidade: string, tipo: string, parcelas: number): number {
    if (modalidade.toLowerCase().startsWith('d')) return 1;
    if (tipo.toLowerCase().includes('vista')) return 2;
    if (parcelas <= 6) return 3;
    if (parcelas <= 12) return 4;
    return 5;
  }
  ...
  protected submitCsv(): void {
    ...
    const submitNext = (index: number) => {
      ...
      const establishment = this.establishmentFacade
        .activeOptions()
        .find((e) => e.pvNumber === group.pvNumber);
      ...
      this.saleSummaryFacade
        .createManual({
          ...
          transactions: group.transactions.map((tx) => ({
            ...
            discountValue:
              tx.discountValue != null && tx.discountValue > 0 ? tx.discountValue : null,
  ...
  private createCreditOrders(
    summaryId: string,
    installmentTotal: number,
  ): Observable<CreditOrderManualResult[]> {
    const total = Math.max(installmentTotal, 1);
    return from(Array.from({ length: total })).pipe(
      concatMap(() => this.creditOrderFacade.createManual({ summaryIds: [summaryId] })),
      toArray(),
    );
  }
  ...
  private mapTransaction(
    ...
    const flagName = tx.flagId
      ? (this.flagFacade.options().find((f) => f.id === tx.flagId)?.name ?? null)
      : null;
    return {
      nsu: tx.nsu,
